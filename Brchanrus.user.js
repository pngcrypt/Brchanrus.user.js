...
replacer.cfg["main"] = [
	...
	// Админка - Настройки доски - Тэги
	[/^mod\.php\?\/tags/, [
		['reg', 'head > title, header > h1', 'Editar tags', 'Редактирование тегов', [RE_MULTI]],
		['css', 'body > div > form', [
			['reg', '> p', [
				[/^Leia com atenção.+/, 'Внимание: Эти параметры помогают пользователям найти свою доску с помощью тегов.'],
				[/^Se a sua board conter conteúdo.+/, 'Если ваша доска имеет содержание +18 (контент для взрослых), но помечена "не только для взрослых", то доступ к этой доске может быть закрыт.'],
				[/^Por favor, certifique-se.+/, 'Пожалуйста, убедитесь, что не поставили неправильные метки. Если мы увидим, что ваша доска содержит теги, не соответствующие содержанию контента, мы можем заблокировать доступ к странице настроек доски и вы не сможете редактировать теги.']
			]],
			['reg', '> table > tbody > tr > th', [
				['Não possui conteúdo adulto?', 'У вас нет контента для взрослых?'],
				[/Tag (\d+)/, 'Тег $1', [RE_MULTI]]
			]],
			['att', 'input[type="submit"]', 'value', ['Atualizar tags', 'Обновить теги']]
		]],
		[]
	]],
...
replacer.cfg["alert"] = [
	['', [
		...
		['str', 'O tópico especificado não existe', 'Данный тред не существует']
	]]
];
