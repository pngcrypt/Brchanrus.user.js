...
// _d - день; _n - месяц; _y - год (2 цифры); _Y - год (4 цифры); _w - день недели (сокр.); _h - часы; _i - минуты; _s - секунды
const TIME_FORMAT = [
	"", // резерв
	"_d/_n/_y", // только дата
	"_h:_i:_s", // только время
	"_d/_n/_y (_w) _h:_i:_s" // дата + время
];
...
const RE_TIME = 50; /* флаг наличия в regex шаблона для коррекции и замены времени;
	шаблон должен иметь вид: (:<T>d n y h i s) -- :<T> обязательно, дальше идут буквы, указывающие порядок следования цифр в искомой дате/времени
	возможно указывать только время или только дату: (:<T>h i s) или (:<T>d n y) 
	буквы в шаблоне заменяются на \d+, пробелы - на \D+
	в строке замены дата подставляется вместо кода $T
	возможно использовать только 1 шаблон в regex - остальные игнорируются
*/
...
replacer.cfg["main"] = [
	...
	[/^/, 'body > div.ban.oficial > h2', [
		...
		['reg', 'div#noticias > div > h2 > span' , /at (:<T>d n y h i s)/, '$T', [RE_TIME, RE_MULTI]],
	...
	[/^mod\.php\?\/$/, [
		...
		['reg', 'fieldset > ul > li > ul > li > small', /at (:<T>d n y h i s)/, '$T', [RE_MULTI, RE_TIME]],
...
*/
	...
	for(let r of rx_arr) {
		...
		if(opt.time && !RE.time) {
			// regex содержит шаблон времени и он еще не проинициализирован 
			RE.time = {rx: "", rx_group:0, groups: [], format: 0};
			let brackets = (r[0].source || r[0]).split('('); // разбиваем исходный regex на группы по скобкам
			for(let i = 1; i < brackets.length; i++) {
				// ищем шаблон времени (:<T>...) в группах и меняем его на (\d+\D+...)
				brackets[i] = brackets[i].replace(/^:<T>([^)]+)/, function(s, g1) { 
					let sout = "";
					RE.time.rx_group = i; // номер группы шаблона времени в исходном regex
					g1.replace(/([Yyndhis ])/g, function(s, g1) { // формируем шаблоны поиска и захвата 
						if(g1 == " ") {
							RE.time.rx += "\\D+";
							sout += "\\D+";
						}
			...
			r[0] = new RegExp(brackets.join('(')); // формируем новый regex
		} // if opt.time
		...
		if( (matches=str.match(r[0])) ) {
			...
			if(RE.time && RE.time.rx_group) {
				...
				let m = matches[RE.time.rx_group].match(RE.time.rx); // захватываем цифры из найденного совпадения
				if(m) {
					time.setTime(0);
					let val;
					for(let i=0; i<RE.time.groups.length; i++) {
						val = m[i+1];
					...
					s_rep = s_rep.replace('$T', main.timeFormat(time, false, RE.time.format)); // в строке замены меняем $T на форматированное время
				}
			}
			callback_set(str.replace(r[0], s_rep), opt);
			dbgMsg += ": FOUND";
			if(opt.single) {
...
var main = {
	...
	// ----------------------------------------------------
	{
		...
		*/
		...
		let s = "";
		let delim = false;
		for(let c of TIME_FORMAT[format || 3]) {
			if(delim) {
				delim = false;
				switch(c) {
