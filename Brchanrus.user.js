...
const RE_TIME = 50; /* флаг наличия в regex шаблона для коррекции времени;
	шаблон должен иметь вид: (:<T>d n y h i s)  после :<T> идут буквы, указывающие порядок следования цифр в искомой дате/времени
	...
	также можно указать номер предустановленного шаблона: (:<T0>), (:<T1>) - паттерн д.б прописан в TIME_PATTERN
	исполь regex - остальные игнорируются
*/
				...
				brackets[i] = brackets[i].replace(/^:<T(\d*)>([^)]*)/, function(s, g1, g2) { 
					...
					if(patt) {
						...
						if(patt.time) {
							...
							RE.time = patt.time;
							RE.time.rx_group = i; // корректируем номер группы для тек. regex
							return patt.find_pattern;
						}
						g2 = patt[0]; 
					}
